...
import CategoryRestaurantGrid from './CategoryRestaurantGrid';

function CategoryHero() {
  const { type } = useParams();
  const { title, description, text, loading, error } = useCategoryData(type);

  if (loading) {
    return <div className="px-6 py-6 text-center">Loading category data...</div>;
  }

  if (error) {
    return <div className="px-6 py-6 text-center text-red-500">Error: {error}</div>;
  }
  ...
  return (
    <div className="px-6 py-6">
      ...
      <div className='ml-20 mr-10'>
        <CategoryRestaurantGrid type={type} />
      </div>
    </div>
  );
}
